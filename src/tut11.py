...
def plot_chart():
    ...
    if chart_option.get() == "Barchart" and df is not None:
        try:
            ...
            print(f"Error plotting barchart: {e}")
        ...
        try:
            ...
            if row and col and row in df.columns and col in df.columns:
                ...
                plt.figure(figsize=(10, 6))
                m = Basemap(projection='merc',llcrnrlat=-60,urcrnrlat=85,llcrnrlon=-180,urcrnrlon=180,lat_ts=20,resolution='c')
                m.drawcoastlines()
                m.drawcountries()
                m.drawmapboundary(fill_color='aqua')
                m.fillcontinents(color='coral',lake_color='aqua')

                lons = valid_data[col].tolist()
                lats = valid_data[row].tolist()
                x, y = m(lons, lats)
            ...
            print(f"Error plotting scattermap: {e}")
