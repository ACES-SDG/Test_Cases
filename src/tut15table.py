...
drag_window = None
df = None
agg_function = tk.StringVar(value="mean")
...
def on_drag_release(event):
    ...
    for drop_target in [row_area, column_area]:
        target_x, target_y, target_width, target_height = (drop_target.winfo_rootx(), drop_target.winfo_rooty(), 
                                                          drop_target.winfo_width(), drop_target.winfo_height())
...
def plot_chart():
    if chart_option.get() == "Barchart" and df is not None:
        ...
        if row and col and row in df.columns and col in df.columns:
            ...
            canvas.draw()
    elif chart_option.get() == "Table" and df is not None:
        display_table()
...
chart_option = ttk.Combobox(control_frame, values=["Barchart", "Table"])
